...
if len(victory_names) > 1:
    ...
    manual_options['goal'].__doc__ = "Choose your victory condition.\n# \n# Defeat Ghetsis - (Ideal for Asyncs) Collect all eight Badges and defeat the elite four followed by N and Ghetsis.\n# Defeat Cobalion - (Sync friendly option) Collect five Badges and defeat Cobalion at Mistralton Cave 3F.\n# Defeat Cynthia - (Requires Defeating Ghetsis) Confront Cynthia in Undella Town.\n# Become Champion - (Requires Defeating Ghetsis) Collect all Seven Sage's Wills and arrest the scattered Sages, and then Defeat Alder.\n# All locations accessible after Route 7 (including areas you backtrack to) will be removed if your goal is to defeat Cobalion."
...
for category in category_table:
    for option_name in category_table[category].get("yaml_option", []):
        ...
        if option_name not in manual_options:
            manual_options[option_name] = type(
                option_name, (DefaultOnToggle,), {"default": True})
...
manual_options_data = make_dataclass(
    'ManualOptionsClass', manual_options.items(), bases=(PerGameCommonOptions,))
